...
class RobotControlTab(QWidget):
    ...
    def __init__(self, robot_service: RobotService, parent=None):
    ...
    def create_connection_control_panel(self):
        ...
        group.setMaximumHeight(104)  # 增加高度30%: 80 * 1.3 = 104
        layout = QVBoxLayout()
    ...
    def create_enhanced_path_management(self):
        ...
        # 路径表格 - 移除最大高度限制，让其占用剩余空间
        self.path_table = QTableWidget()
        self.path_table.setColumnCount(6)
        self.path_table.setHorizontalHeaderLabels(["路径名称", "路径点数", "创建时间", "描述", "状态", "操作"])
        # 修改表格缩放行为：不拉伸最后一列，让表格填充空间
        self.path_table.horizontalHeader().setStretchLastSection(False)
        # 设置各列的缩放模式，让表格填充可用空间
        header = self.path_table.horizontalHeader()
        header.setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)  # 路径名称列拉伸
        header.setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)  # 点数列适应内容
        header.setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)  # 创建时间列适应内容
        header.setSectionResizeMode(3, QHeaderView.ResizeMode.Stretch)  # 描述列拉伸
        header.setSectionResizeMode(4, QHeaderView.ResizeMode.ResizeToContents)  # 大小列适应内容
        header.setSectionResizeMode(5, QHeaderView.ResizeMode.ResizeToContents)  # 操作列适应内容
        self.path_table.setSelectionBehavior(QTableWidget.SelectionBehavior.SelectRows)
        self.path_table.setMinimumHeight(150)  # 设置最小高度
    ...
    def test_robot_connection(self):
        """测试机械臂连接"""
        info("用户点击测试连接按钮", "ROBOT_UI")
        self.add_robot_log("信息", "用户点击测试连接按钮")

        if self.robot_service.is_connected():
            info("执行机械臂连接测试", "ROBOT_UI")
            self.add_robot_log("信息", "执行机械臂连接测试")

            result = self.robot_service.test_connection()
            if result['success']:
                device_type = result.get('device_info', {}).get('type', '未知')
                info(f"机械臂连接测试成功，设备信息: {device_type}", "ROBOT_UI")
                self.add_robot_log("信息", f"机械臂连接测试成功，设备信息: {device_type}")
                QMessageBox.information(self, "测试成功",
                    f"机械臂连接测试成功！\n"
                    f"设备信息: {device_type}")
            else:
                error_msg = result.get('error', '未知错误')
                warning(f"机械臂测试失败: {error_msg}", "ROBOT_UI")
                self.add_robot_log("警告", f"机械臂测试失败: {error_msg}")
                QMessageBox.warning(self, "测试失败",
                    f"机械臂连接测试失败！\n错误信息: {error_msg}")
        else:
            # 设备未连接，显示提示
            warning("设备未连接，无法执行测试", "ROBOT_UI")
            self.add_robot_log("警告", "设备未连接，无法执行测试")
            QMessageBox.warning(self, "测试失败", "请先连接机械臂后再进行测试")
    ...
    def jog_move(self, direction: str):
        ...
        try:
            ...
            current_pos_str = f"({current_pos[0]:.1f}, {current_pos[1]:.1f}, {current_pos[2]:.1f})" if current_pos else "未知"
        except:
            current_pos_str = "获取失败"

        # 从service层执行点动移动
        movement_info = f"执行点动移动: 轴{axis}, 方向:{'+' if direction_value > 0 else '-'}, 速度:{speed}%, 距离:{distance}mm"
        info(f"开始{movement_info}", "ROBOT_UI")
        ...
        if result['success']:
            ...
            error_msg = result.get('error', '未知错误')
            error(f"点动移动失败: {error_msg}", "ROBOT_UI")
    ...
    def move_to_position(self):
        ...
        if result['success']:
            ...
            info(f"位置移动命令已发送到service层", "ROBOT_UI")
            ...
            error_msg = result.get('error', '未知错误')
            error(f"位置移动失败: {error_msg}", "ROBOT_UI")
    ...
    def go_home(self):
        ...
        if result['success']:
            ...
            error_msg = result.get('error', '未知错误')
            error(f"回原点操作失败: {error_msg}", "ROBOT_UI")
            self.add_robot_log("错误", f"回原点操作失败: {error_msg}")

    def emergency_stop(self):
        """紧急停止"""
        info("用户触发紧急停止", "ROBOT_UI")
        ...
        if result['success']:
            ...
            error_msg = result.get('error', '未知错误')
            warning(f"紧急停止失败: {error_msg}", "ROBOT_UI")
